...
class TouchButton():
    ...
    def captureAndCheckPicCount(self,capturemode,delaytime=0):
        ...
        beforeNo = commands.getoutput('adb shell ls /sdcard/DCIM/101ANDRO/* | grep '+ d[capturemode] +' | wc -l') #Get count before capturing
        if capturemode == 'video':
            self.takeVideo(delaytime)
        else:
            self.takePicture(capturemode)
        time.sleep(delaytime) #Sleep a few seconds for file saving
        afterNo = commands.getoutput('adb shell ls /sdcard/DCIM/101ANDRO/* | grep '+ d[capturemode] +' | wc -l') #Get count after taking picture
